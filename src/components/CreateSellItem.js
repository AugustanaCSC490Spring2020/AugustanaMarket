...
const SellItem = ({ match, history }) => {
    const [ requestOrSell, setRequestOrSell ] = useState('');
    ...
    if (!((createType === 'request' && requestOrSell === 'buy') || createType === requestOrSell)) {
    ...
    const finishSellItem = () => {
        ...
        firebase.firestore().collection(requestOrSell === 'sell' ? 'sell' : 'buy').add(data).then((doc) => {
            if (images !== '') {
